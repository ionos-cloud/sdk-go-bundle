...
// APIClient manages communication with the Activity Log Service API API vv1
...
func NewAPIClient(cfg *shared.Configuration) *APIClient {
	...
	*cfgCopy = *cfg
	...
	if len(cfgCopy.Servers) == 0 {
		...
		shared.LogDebug("[DEBUG] activitylog: using default endpoint: %s", cfgCopy.Servers[0].URL)
	} else {
		shared.LogDebug("[DEBUG] activitylog: server config provided with %d endpoint(s)", len(cfgCopy.Servers))
	...
	for i, srv := range cfgCopy.Servers {
		shared.LogDebug("[DEBUG] activitylog: final endpoint[%d]: url=%s description=%q", i, srv.URL, srv.Description)
	}
...
func (c *APIClient) callAPI(request *http.Request) (*http.Response, time.Duration, error) {
	...
	for {
		...
		if err != nil {
			shared.LogDebug("[DEBUG] activitylog: request failed for %s %s: %v", clonedRequest.Method, clonedRequest.URL.Host, err)
		...
		case http.StatusServiceUnavailable,
			...
			if request.Method == http.MethodPost {
				shared.LogDebug("[DEBUG] activitylog: received %d for POST %s, not retrying (non-idempotent)", resp.StatusCode, request.URL.String())
				return resp, httpRequestTime, err
			}
			shared.LogDebug("[DEBUG] activitylog: received %d for %s %s, will retry (attempt %d/%d)", resp.StatusCode, request.Method, request.URL.String(), retryCount, c.GetConfig().MaxRetries)
		...
		case http.StatusTooManyRequests:
			if retryAfterSeconds := resp.Header.Get("Retry-After"); retryAfterSeconds != "" {
				...
				shared.LogDebug("[DEBUG] activitylog: rate limited (429) for %s %s, retry-after=%ss (attempt %d/%d)", request.Method, request.URL.String(), retryAfterSeconds, retryCount, c.GetConfig().MaxRetries)
			} else {
				backoffTime = c.GetConfig().WaitTime
				shared.LogDebug("[DEBUG] activitylog: rate limited (429) for %s %s, using default backoff (attempt %d/%d)", request.Method, request.URL.String(), retryCount, c.GetConfig().MaxRetries)
			}
		default:
			return resp, httpRequestTime, err
		...
		if retryCount >= c.GetConfig().MaxRetries {
			shared.LogDebug("[DEBUG] activitylog: retry exhausted after %d attempts for %s %s, last status=%d", retryCount, request.Method, request.URL.String(), resp.StatusCode)
			break
		} else {
