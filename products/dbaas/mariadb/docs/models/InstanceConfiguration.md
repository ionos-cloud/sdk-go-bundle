...
|------------ | ------------- | ------------- | -------------|
|**Count** | **int32** | The total number of instances in the cluster. A value of 1 creates a single-instance cluster. Values from 2 to 5 create a replicated cluster with one primary and n-1 secondary instances.  | |
|**Ram** | **int32** | The amount of memory (RAM) per instance in gigabytes (GB). On update, RAM can be increased or decreased. | |
|**Cores** | **int32** | The number of CPU cores per instance. On update, cores can be increased or decreased. | |
|**StorageSize** | **int32** | The amount of storage per instance in gigabytes (GB). On update, storage size can only be increased; it cannot be reduced.  | |
